...
from .utils import Dispatcher, Supabase, delete, post
from .row import Row
...
class CatalogBase(Dispatcher, Supabase):
    ...
    def register(self, name: str, endpoint: str = None, token: str = None, **kwargs):
        ...
        if len(objects) == 0:
        ...
        elif objects[0].token in (None, token):
            values = {
                "timestamp": time.time(),
                "token": token
            }
            ...
            self.update(self._table, values, objects[0].filter("name"))
    ...
    def unregister(self, name: str, token: str = None, **kwargs):
        ...
        elif objects[0].token is not None and objects[0].token != token:
            raise cherrypy.HTTPError(401, "Invalid token.")
        else:
